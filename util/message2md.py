...
def single_message2md(message_info=None):
    ...
    # 修改为当前项目目录下的路径
    img_path = Path(__file__).parent.parent / 'data' / 'frontcover'
    md_path = Path(__file__).parent.parent / 'data' / 'posts'
...
top: false
hide: false
img: frontcover/{_id.replace('/', '_')}.jpg
tags: 
    - 开源项目
    - 微信公众号聚合平台
    - {d['oaname']}
...
'''
        ...
        # 检查文章内容是否存在，不存在则跳过
        if _id not in message_detail_text:
            # 创建一个空条目
            message_detail_text[_id] = "文章内容未获取"
            # 跳过剩余处理
            continue
        ...
        md_file_path = os.path.join(md_path, f"{_id.replace('/', '_')}.md")
        with open(md_file_path, 'w', encoding='utf-8') as f:
            f.write(md)

    # 保存更新后的message_detail_text
    handle_json('message_detail_text', data=message_detail_text)
