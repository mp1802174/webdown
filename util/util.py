...
def handle_json(filename, data=None, mode='r', path='data/'):
    filepath = Path(__file__).resolve().parent.parent / path / f'{filename}.json'
    if mode == 'r':
        if not filepath.exists():
            # print(f"File {filepath} does not exist, returning empty dict.") # Optional debug
            return {}
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                content = f.read()
                if not content.strip(): # Check if file is empty or only whitespace
                    # print(f"File {filepath} is empty or whitespace-only, returning empty dict.") # Optional debug
                    return {}
                return json.loads(content) # Use json.loads on the read content
        except json.JSONDecodeError:
            print(f"Warning: JSONDecodeError while reading {filepath}. File content might be malformed. Returning empty dict.")
            return {}
        except Exception as e:
            print(f"Warning: An unexpected error occurred while reading {filepath}: {e}. Returning empty dict.")
            return {}
    elif mode == 'w':
        # 安全写入，防止在写入过程中中断程序导致数据丢失
        with open('tmp.json', 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=4)
        shutil.move('tmp.json', filepath)
